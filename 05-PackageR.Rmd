...
#'
#' @return A vector of the same length as `number` containing the transformed values.
#' @export
#'
#' @examples
#' double(2)
...
Ensuite, contruire le package (*Install and Restart*) et vérifier le résultat en exécutant la fonction et en affichant son aide:
```{r, eval=FALSE}
double(2)
?double
```
...
Il est temps de lancer une vérification de son code: dans la fenêtre *Build*, cliquer sur "Check".
...
Le fichier `package.R` est destiné à recevoit le code R et surtout les commentaires pour **roxygen2** qui concernent l'ensemble du package.
...
Après reconstuction du package, vérifier que l'aide est est apparue: `?multiple`.


### Vignette

Les vignettes sont la documentation complète du package.
...
Le reste de l'entête permet à R de construire la vignette à partir de code RMarkdown.
...
Une introduction à l'utilisation du package doit être écrite dans ce documents, en RMarkdown.
...
* En ajoutant des articles au format RMarkdown dans le dossier `vignettes/articles`;
...
[^506]: adv-r.had.co.nz/OO-essentials.html
...
Comme un objet peut appartenir à plusieurs classe, il recherche une fonction adaptée à la première classe, puis aux classes suivantes successivement.
Cette recherche prend beaucoup de temps et justifie de réserver l'usage de méthodes génériques à la clarification du code plutôt qu'à une recherche de performance.
...
#' @source Values computed by the R software, \url{https://www.r-project.org/}
"seq1_10"
```
...
## Intégration continue {#sec:package-ci}
