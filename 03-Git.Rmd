...
GitHub n'est pas la seule plateforme utilisable mais la principale. Les alternatives sont Bitbucket[^309] et GitLab[^310] par exemple.
...
La branche principale du projet s'appelle "master" ou "main" (à partir de fin 2020).
...
Cliquer sur le bouton *New Branch* dans la fenêtre *git* de RStudio.
...
Quand la branche de développement est stabilisée, il est temps d'intégrer ses changements à la branche principale par une fusion.
...
La branchea fusionnée n'est pas supprimée: elles peut être utilisée à nouveau pour d'autres développements ou supprimée manuellement avec la commande suivante: 
```
...
## Activer les pages GitHub  {#sec:github-pages}

Tout projet sur GitHub doit avoir contenir un fichier `README.md` pour le présenter.
Ce fichier est écrit au format Markdown.

Le fichier peut être dupliqué dans le dossier `docs` pour fournir la page d'accueil du site web du dépôt.
Un dépôt contenant un mémo écrit en R Markdown (voir section \@ref(sec:memo)) est utilisé comme exemple[^306].

[^306]: https://github.com/EricMarcon/Krigeage

Son fichier `README.md` existe aux deux emplacements.
