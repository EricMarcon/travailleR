...
Le script minimal pour la vérification d'un package est le suivant[^ci-5]:
...
Le fichier est nommé `CMD-check.yml`.
...
Enfin, l'étape `Check` vérifie le package.
Les avertissements sont traités comme des erreurs.

Ce script ne produit rien: son succès est affiché par un badge à afficher dans le fichier `README.md` (voir section \@ref(sec:ci-badges))
