...
Un script simple pour un projet `bookdown` ressemble à ceci:
...
r_packages:          # Packages à installer pour Travis
  ...
  - pkgdown          # Pour la création du site d'aide des packages R
```
...
```
after_success:
  - Rscript -e 'covr::codecov(type ="all")'            # Appel à Covecov
  - mkdir docs                                         # Dossier du site d'aide
  - git fetch origin gh-pages:gh-pages                 # Tirer...
  - git --work-tree=docs checkout gh-pages -- .        # ...dans docs
  - travis_wait 60 Rscript -e 'pkgdown::build_site()'  # Construction du site
  - touch docs/.nojekyll                               # Fichier vide
```
...
notifications:
  email:
    recipients:
      - e.marcon@free.fr   # Notification par mail après exécution
```
