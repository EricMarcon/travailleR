...
Le plus courant est S3, mais ce n'est pas le seul.
...
# Construction d'une instance, i.e. 
...
Les méthodes appartiennent aux classes, comme en C++: elles sont déclarées dans la classe et appelées à partir des obkets.
...
En supprimant la variable de l'environnement global par `rm(q)`, la fonction `qConsole()` parcourt la pile des environnements jusqu'à trouver un objet nommé `q` dans le package **base**, qui est la fonction permettant de quitter R.
...
L'appel de `devtools::dr_devtools()` charge le package (c'est-à-dire, exécute implicitement `loadNamespace("devtools")`) mais ne l'attache pas.
...
Cependant, le package **dplyr** ou **tidyverse** (qui attache tous les packages du tidyverse) peut être chargé systématiquement en créant un fichier `.RProfile` à la racine du projet:
```{r, eval=FALSE}
library("tidyverse")
```
...
Ce document est tricoté avec ```r  parallel::detectCores()``` coeurs, qui sont tous utilisés.
...
nbCoeurs <- detectCores()
# Série : temps théorique = nbCoeurs/4 secondes
(tserie <- system.time(lapply(1:nbCoeurs, f)))
# Parallèle : temps théorique = 1/4 secondes
(tparallele <- system.time(mclapply(1:nbCoeurs, f)))
```

La mise en place de la parallélisation a un coût de l'ordre du temps de calcul au-delà du quart de seconde attendu: environ ```r format(tparallele[3]-1/4, digits=2)``` secondes ici.
...
# Parallèle
system.time(mclapply(1:nbCoeurs, f, time=1))
```
...
```{r}
system.time(mclapply(1:nbCoeurs, f, time=1))
system.time(mclapply(1:(nbCoeurs+1), f, time=1))
```
...
(ref:r-parallele) Temps d'exécution en parallèle de tâches nécessitant un quart de secondes (chaque tâche est une pause d'un quart de seconde). Le nombre de tâches varie de 1 à deux fois le nombre de coeurs (égal à ```r  parallel::detectCores()```) plus une. 
```{r r-parallele, fig.cap="(ref:r-parallele)", echo=FALSE}
Taches <- 1:(2*nbCoeurs+1)
Temps <- sapply(Taches, function(nbTaches) system.time(mclapply(1:nbTaches, f)))
library("tidyverse")
