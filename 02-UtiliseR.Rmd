...
Chacune de ces trois opération est une cible dans le vocabulaire de **targets**. 
...
Chacune est déclarée par la fonction `tar-target` dont le premier argument est le nom de la cible, qui sera celui de la variable qui recevra le résultat.
...
Quand ce n'est pas le cas, chaque cible peut être écrite sous la forme d'une fonction, stockée dans un fichier de code chargé par la fonction `source()` au début du fichier de flux.
