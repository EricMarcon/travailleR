...
(ref:r-parallele) Temps d'exécution en parallèle de tâches nécessitant une seconde (chaque tâche est une pause d'un quart de seconde). Le nombre de tâches varie de 1 à deux fois le nombre de coeurs utilisés (égal à ```r nbCoeurs```) plus une. 
```{r r-parallele, fig.cap="(ref:r-parallele)"}
Taches <- 1:(2*nbCoeurs+1)
Temps <- sapply(Taches, function(nbTaches) system.time(mclapply(1:nbTaches, f, time=1, mc.cores=nbCoeurs)))
library("tidyverse")
