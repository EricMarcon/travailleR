...
# Logarithme de base 2 de la moyenne de 100 tirages aléatoires 
# dans une loi uniforme
...
mb <- microbenchmark(
  ...
  for (i in 1:length(x1)) Racine[i] <- sqrt(x1[i])
)
summary(mb)[, c("expr", "median")]
```
...
#| fig.scap: "Temps d'exécution en parallèle"
Taches <- 1:(2 * nbCoeurs + 1)
Temps <- sapply(
  Taches, 
  FUN = function(nbTaches) {
...
tibble(Taches, Temps = Temps["elapsed", ]) %>% 
  ...
  geom_vline(xintercept = (1:2) * nbCoeurs, col = "red", lty = 2)
```
...
# Série
system.time(
  foreach(i = icount(nbCoeurs), .combine = "c") %do% {f(i)}
)
# Parallèle
system.time(
  foreach(i = icount(nbCoeurs), .combine = "c") %dopar% {f(i)}
)
```
...
library("doFuture")
system.time(
  foreach(i = icount(nbCoeurs), .combine = "c") %dofuture% {f(i)}
)
```
...
fsapply5 <- function() {
  ...
  return(
    sum(sqrt(simplify2array(distances))) / NbPoints / (NbPoints - 1) * 2
  )
}
...
Ce temps est à comparer à celui de la boucle for de référence, multiplié par 2500, soit ```r format(as.integer(for_time["elapsed"]*2500), digits=1)``` secondes.
...
Le package **targets** permet de gérer un flux de travail (*workflow*), c'est-à dire de décomposer le code en tâches élémentaires appelées *cibles* qui s'enchaînent, dont le résultat est stocké dans une variable, elle-même enregistrée sur le disque.
...
**targets** permet de visualiser les tâches obsolètes, d'intégrer le flux à un projet de document (voir section \@ref(sec:targetsmd)), et même de faire appel à un cluster de calcul pour traiter les tâches en parallèle.

### Principe de fonctionnement

La documentation[^241] de **targets** est détaillée et fournit un exemple travaillé pour apprendre à utiliser le package[^242].
...
Si le projet nécessite de longs calculs, **targets** permet de n'exécuter que ceux qui sont nécessaires.
...
L'exemple suivant est encore plus simple que celui du manuel de **targets**, qui permettra d'aller plus loin.
...
Chacune de ces trois opérations est une cible dans le vocabulaire de **targets**. 
...
#| include: false
# Ecriture sur le disque du fichier _targets.R
...
# Destruction des exécutions précédentes pour la démonstration
targets::tar_destroy()
```

Les commandes globales consistent à charger le package **targets** lui-même puis lister les packages nécessaires au code.
