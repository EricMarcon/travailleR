...
Les versions d'étape sont déposées sur HAL: \url{https://hal.archives-ouvertes.fr/}.]
...
L'optimisation de l'utilisation des nombreux outils disponibles est traitée en détail: \textbf{rmarkdown}, \textbf{bookdown} et \textbf{blogdown} pour la rédaction, \textbf{roxygen2}, \textbf{testthat} et \textbf{pkgdown} pour les packages, le contrôle de source avec git et GitHub, l'intégration continue avec Travis et Codecov.
...
\begin{verbatim}
## [1] "/home/travis/build/EricMarcon/travailleR"
\end{verbatim}
...
##  f1(X)   8.222   9.4340  19.53909  10.071  10.8580  940.560   100
##  f2(X)  10.401  11.4740  22.23717  11.981  12.4850 1001.005   100
##  f3(X) 459.809 461.3615 476.50695 463.653 465.9045 1533.619   100
##  f4(X) 773.197 777.0065 793.51745 778.287 781.5825 1946.054   100
\end{verbatim}
...
##                                    expr   median
## 1                              sqrt(x1)   3.1975
## 2                lapply(x1, FUN = sqrt) 256.4775
## 3                sapply(x1, FUN = sqrt) 309.5225
## 4 vapply(x1, FUN = sqrt, FUN.VALUE = 0) 260.4805
\end{verbatim}
...
\begin{verbatim}
##                        expr   median
## 1 vapply(x1, FUN = sqrt, 0) 252.1055
## 2                Boucle(x1)  65.7910
\end{verbatim}
...
\begin{verbatim}
##                                               expr    median
## 1                        vapply(x1, FUN = sqrt, 0)  270.4565
## 2 for (i in 1:length(x1)) Racine[i] <- sqrt(x1[i]) 2375.1385
\end{verbatim}
...
\begin{verbatim}
##                        expr   median
## 1 replicate(1000, runif(1)) 2185.008
## 2               runif(1000)   27.893
\end{verbatim}
...
\begin{verbatim}
##                              expr   median
## 1   BoucleSomme(diamonds[-(2:4)]) 1.909765
## 2 apply(diamonds[-(2:4)], 2, sum) 4.335472
## 3       colSums(diamonds[-(2:4)]) 1.293880
\end{verbatim}
...
\NormalTok{mclapply.hack <-}\StringTok{ }\ControlFlowTok{function}\NormalTok{(...) \{}
...
\NormalTok{  cl <-}\StringTok{ }\KeywordTok{makeCluster}\NormalTok{( }\KeywordTok{min}\NormalTok{(size.of.list, }\KeywordTok{detectCores}\NormalTok{()) )}
...
Ce document est tricoté avec \texttt{2} coeurs, qui sont tous utilisés.
...
\NormalTok{\}}
\NormalTok{nbCoeurs <-}\StringTok{ }\KeywordTok{detectCores}\NormalTok{()}
...
\NormalTok{(tparallele <-}\StringTok{ }\KeywordTok{system.time}\NormalTok{(}\KeywordTok{mclapply}\NormalTok{(}\DecValTok{1}\OperatorTok{:}\NormalTok{nbCoeurs, f)))}
\end{Highlighting}
\end{Shaded}
...
La mise en place de la parallélisation a un coût de l'ordre du temps de calcul au-delà du quart de seconde attendu: environ \texttt{0.009} secondes ici.
...
\KeywordTok{system.time}\NormalTok{(}\KeywordTok{mclapply}\NormalTok{(}\DecValTok{1}\OperatorTok{:}\NormalTok{nbCoeurs, f, }\DataTypeTok{time =} \DecValTok{1}\NormalTok{))}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{system.time}\NormalTok{(}\KeywordTok{mclapply}\NormalTok{(}\DecValTok{1}\OperatorTok{:}\NormalTok{(nbCoeurs }\OperatorTok{+}\StringTok{ }\DecValTok{1}\NormalTok{), f, }\DataTypeTok{time =} \DecValTok{1}\NormalTok{))}
\end{Highlighting}
\end{Shaded}
...
\caption{Temps d'exécution en parallèle de tâches nécessitant un quart de secondes (chaque tâche est une pause d'un quart de seconde). Le nombre de tâches varie de 1 à deux fois le nombre de coeurs (égal à \texttt{2}) plus une.}\label{fig:r-parallele}
\end{SCfigure}
...
\NormalTok{\}}
\KeywordTok{system.time}\NormalTok{(d <-}\StringTok{ }\KeywordTok{ffor}\NormalTok{())}
\end{Highlighting}
\end{Shaded}
...
## Unit: microseconds
##                                                      expr     min      lq
##  d <- TotalDistance(X$x, X$y)/NbPoints/(NbPoints - 1) * 2 909.188 912.349
##      mean   median       uq      max neval
##  943.8193 913.7985 921.6715 2241.107   100
\end{verbatim}
...
\begin{verbatim}
## v Setting active project to '/home/travis/build/EricMarcon/travailleR'
\end{verbatim}

\begin{verbatim}
## * Copy and paste the following lines into '/home/travis/build/EricMarcon/travailleR/README.md':
\end{verbatim}
...
\caption{(ref:rediger-rediger-Projet-GitHub)}\label{fig:rediger-Projet-GitHub}
\end{SCfigure}
...
La ligne \texttt{edit\_page} doit être mise à jour: remplacer le dépôt par défaut ``\url{https://github.com/gcushen/hugo-academic}'' par celui du site.

``Contact details'' contient les informations pour contacter le propriétaire du site.
Elles doivent être saisies.

``Regional Settings'' contient les paramètres d'affichage de date pour la langue par défaut (ceux des autres langues sont dans \texttt{languages.toml}).
...
``Content Management System'' contient la ligne \texttt{netlify\_cms} dont la valeur doit être \texttt{false} si le site n'est pas hébergé par Netlify.
...
Le composant \texttt{demo.md} (figure \ref{fig:rediger-demo}) est un composant de type ``blank'', c'est-à-dire une page de texte libre: il sert ici à présenter le modèle Academic Kickstart et doit donc être désactivé.
...
\begin{Highlighting}[]
\NormalTok{knitr}\OperatorTok{::}\NormalTok{opts_chunk}\OperatorTok{$}\KeywordTok{set}\NormalTok{(}
  \DataTypeTok{cache=}\OtherTok{TRUE}\NormalTok{, }\DataTypeTok{warning=}\OtherTok{FALSE}\NormalTok{,}
  \DataTypeTok{echo =} \OtherTok{TRUE}\NormalTok{,}
  \DataTypeTok{fig.env=}\StringTok{'SCfigure'}\NormalTok{, }\DataTypeTok{fig.asp=}\NormalTok{.}\DecValTok{75}\NormalTok{, }\DataTypeTok{fig.align=}\StringTok{'center'}\NormalTok{, }\DataTypeTok{out.width=}\StringTok{'80%'}\NormalTok{, }
  \DataTypeTok{dev =} \StringTok{"ragg_png"}\NormalTok{,}
  \DataTypeTok{tidy=}\OtherTok{TRUE}\NormalTok{, }\DataTypeTok{tidy.opts=}\KeywordTok{list}\NormalTok{(}\DataTypeTok{blank=}\OtherTok{FALSE}\NormalTok{, }\DataTypeTok{width.cutoff=}\DecValTok{55}\NormalTok{), }\DataTypeTok{size=}\StringTok{"scriptsize"}\NormalTok{,}
  \DataTypeTok{knitr.graphics.auto_pdf =} \OtherTok{TRUE}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
Ils doivent respecter la politique\footnote{\url{https://r-pkgs.org/release.html?q=readme\#readme-rmd}} du dépôt.
...
\begin{verbatim}
## v Leaving 'LICENSE.md' unchanged
\end{verbatim}
...
La démarche est détaillée dans \emph{R Packages}{[}\^{}513{]}.
...
## Unit: nanoseconds
##                expr  min     lq     mean median     uq     max neval
##  triple.integer(2L)  299  331.5 16125.02  363.0  413.0 1572143   100
##   triple.numeric(2)  298  339.0 14252.97  377.5  428.5 1385711   100
##          triple(2L) 1017 1075.5  1237.21 1127.0 1186.0    5515   100
\end{verbatim}
...
\begin{center}\includegraphics[width=0.8\linewidth]{travailleR_files/figure-latex/unnamed-chunk-96-1} \end{center}
...
\begin{center}\includegraphics[width=0.8\linewidth]{travailleR_files/figure-latex/unnamed-chunk-96-2} \end{center}
...
/src-i386/
/src-x64/
# uncomment if pkgdown is run by Travis
# docs/
\end{verbatim}

La dernière ligne concerne le dossier \texttt{docs/}, qui reçoit le site web produit par \textbf{pkgdown}.
Elle est commentée tant que la production du site est réalisée localement, mais décommentée si elle est confiée à Travis-CI (voir section suivante).
...
Travis est également capable de reconstruire la documentation du package avec \textbf{pkgdown}, autre opération consommatrice de ressources, après la réussite des tests.
...
La vérification du package par Travis dans le cadre de l'intégration continue n'est pas suffisante.
...
L'outil utilisé le plus fréquemment pour des projets R déposés sur GitHub est \emph{Travis CI}\footnote{\url{https://travis-ci.org/}}.
...
Pour évaluer le taux de couverture du code des packages R, c'est-à-dire la proportion du code testé d'une façon ou d'une autre (exemples, tests unitaires, vignette), le service \emph{Codecov}\footnote{\url{https://codecov.io/}} s'intègre parfaitement à Travis.
...
Si elles sont produites par Travis, elle le seront obligatoirement dans une branche dédiée appelée \texttt{gh-pages}.

\hypertarget{intuxe9gration-continue-dun-projet-de-document}{%
\section{Intégration continue d'un projet de document}\label{intuxe9gration-continue-dun-projet-de-document}}
...
Générer un nouveau jeton, le décrire en tant que ``Travis'' et lui donner l'autorisation ``repo'', c'est-à-dire modifier \emph{tous} les dépôts (il n'est pas possible de limiter l'accès à un dépôt particulier).
...
\hypertarget{sec:activation-ci}{%
\subsection{Activation du dépôt sur Travis}\label{sec:activation-ci}}
...
{\centering \includegraphics[width=0.8\linewidth]{images/travis-Depots} 

}

\caption{Activation d'un dépôt dans Travis CI.}\label{fig:travis-Depots}
\end{SCfigure}

\normalsize

Cliquer ensuite sur ``Settings'' pour enregistrer le jeton d'accès.
Dans la zone \emph{Environment variables}, compléter:
...
\item
  Name : GITHUB\_TOKEN
\item
  Value : \textless coller le jeton\textgreater{}
\end{itemize}
...
Le nom \emph{GITHUB\_TOKEN} est une convention: on pourrait le modifier, mais on ne le fait pas.
...
L'opération est similaire sur le site Codecov: l'analyse de la couverture du code des packages est utile pour détecter les portions de code non testées.
...
Il est inutile d'utiliser le système de jetons de Codecov pour les projets traités par Travis.

\hypertarget{scripter-les-actions-de-travis}{%
\subsection{Scripter les actions de Travis}\label{scripter-les-actions-de-travis}}

\hypertarget{sec:bookdown-ci}{%
\subsubsection{Projet d'ouvrage}\label{sec:bookdown-ci}}

Le script de contrôle de Travis doit être nommé \texttt{.travis.yml}.
Il est au format YAML.
Un script simple pour un projet \texttt{bookdown} ressemble à ceci (le signe \texttt{\textbar{}\textgreater{}} indique que la suite du code devrait se trouver sur la même ligne, mais est coupée pour le formatage de ce document):
...
Ce script minimal indique que le \emph{langage} est R.

La partie \emph{script} exécute des commandes sous R: \texttt{render\_book} pour construire un livre, aux formats PDF et HTML.
Le fichier PDF doit exister pour que le lien vers son téléchargement soit affiché dans la barre de commandes du format HTML, qui doit donc être créé en dernier.

La partie \emph{deploy} gère le déploiement du résultat, écrit dans le dossier \texttt{docs} de la branche \emph{master} par \texttt{render\_book}.
La commande \texttt{skip\_cleanup} permet d'éviter que le contenu de \texttt{docs} soit effacé entre les phases \texttt{script} et \texttt{deploy}.
La version 2 de l'outil de déploiement de Travis CI supprimera cette commande.

Le fournisseur \texttt{pages} tire la branche \texttt{gh-pages} depuis GitHub, valide les modifications et pousse le résultat.
...
Enfin, l'option \texttt{keep\_history} permet de conserver l'historique de la branche \texttt{gh-pages}.
Elle peut être mise à \texttt{false} pour l'effacer (commande \texttt{git\ push\ -\/-force}) puisque le contenu n'est que le résultat d'une production automatique, l'historique important étant celui du code.
...
Le code correspondant de la partie \texttt{cache}, à placer avant \texttt{script} est le suivant:
...
Si des bouts de code exécutent des calculs longs, il faut augmenter cette limite avec la commande \texttt{travis\_wait} qui reçoit comme premier argument le nombre de minutes permises, ici une heure:
...
Les packages R à installer depuis CRAN peuvent être déclarés avant le script par une entrée \texttt{r\_packages} et une entrée \texttt{r\_github\_packages} pour les packages sur GitHub.
Ce n'est pas une bonne pratique pour les packages du projet: la bonne méthode consiste à utiliser un fichier \texttt{DESCRIPTION}, comme si le projet était un package R.
Les directives \texttt{Depends} et \texttt{Imports} de ce fichier \texttt{DESCRIPTION} sont prises en compte par Travis et installés.

Certains packages nécessitent pour leur installation sous Linux (à partir de leur code source) celle de packages \emph{apt}.
Par exemple, le package R \emph{sf} nécessite l'ajout des entrées suivantes:
...
Pour savoir quels packages \emph{apt} installer, il est nécessaire de se référer à la documentation du package R et prendre en compte les instructions pour l'installation sous Ubuntu.
...
\begin{itemize}
\item
  Fichier \texttt{.travis.yml}:

  language: r

  cache:
  packages: true
  directories: \_bookdown\_files

  script:
  - travis\_wait 60 Rscript -e \textbar\textgreater{}
  ``bookdown::render\_book(`index.Rmd', \textbar\textgreater{}
  `bookdown::gitbook', quiet = TRUE)''
  - travis\_wait 60 Rscript -e \textbar\textgreater{}
  ``bookdown::render\_book(`index.Rmd', \textbar\textgreater{}
  `bookdown::pdf\_book', quiet = TRUE)''
  ...
  branch: master
\item
  Fichier \texttt{DESCRIPTION}:

  Package: travailleR
  Title: Travailler avec R
  Version: 1.0.0
  \href{mailto:Authors@R}{\nolinkurl{Authors@R}}: c(
  person(``Eric'', ``Marcon'', , ``\href{mailto:e.marcon@free.fr}{\nolinkurl{e.marcon@free.fr}}'', \textbar\textgreater{}
  c(``aut'', ``cre''))
  )
  URL: \url{https://github.com/EricMarcon/travailleR}
  Depends:
  R (\textgreater= 3.1.0)
  Imports:
  bookdown,
...
Le fichier \texttt{.travis.yml} est très similaire:
...
L'instruction \texttt{latex:\ false} ajoutée en début de fichier permet d'économiser le temps de l'installation d'une distribution LaTeX puisqu'un site web n'en a normalement pas besoin.

La mise en cache de \texttt{\$HOME/bin} évite d'installer à chaque fois le générateur de site web Hugo.

Le script consiste à installer Hugo avec le package \textbf{blogdown} puis à construire le site (la commande \texttt{build\_site()} est similaire à la commande \texttt{serve\_site()} utilisée pendant le développement mais ne lance pas de serveur web pour afficher le résultat).
Les arguments forcent la construction complète du site.

Le site est créé dans le dossier \texttt{public}.
La commande \texttt{touch} crée un fichier vide appelé \texttt{.nojekyll} qui servira après publication à indiquer à GitHub que le site web créé ne doit pas être traité par son propre générateur de sites appelé Jekyll.

Tout utilisateur de GitHub dispose d'un dépôt par défaut dont le nom est \texttt{GitHubID.github.io}.
Il n'a normalement qu'une seule branche \texttt{master}.
Le contenu de la branche est un site web (pas sa source, qui doit être placée dans un dépôt différent) accessible à l'adresse \url{https://GitHubID.github.io}.
Le déploiement copie le dossier \texttt{public} dans ce dépôt.
Il ne garde pas de trace de l'historique (\texttt{skip\_cleanup:\ true}) puisque le contrôle de source est déjà activé dans le dépôt contenant le code source du site.

Un fichier \texttt{DESCRIPTION} est obligatoire pour Travis qui y cherche les packages R à installer.
Les premières lignes (Package, Title,\ldots) ne sont pas utilisées par Travis mais permettent de respecter le format standard des fichiers \texttt{DESCRIPTION}.
...
Tous les packages utilisés dans les fichiers R Markdown du site doivent être déclarés dans le fichier \texttt{DESCRIPTION}.
...
Pour cela, afficher les propriétés du dépôt sur Travis-CI (voir la saisie du jeton GitHub en section \ref{sec:activation-ci}) et ajouter une tâche programmée (\emph{cron job}).
...
Travis s'appuie sur le fichier \texttt{DESCRIPTION} pour installer les packages nécessaires et lance la vérification du package avec l'option \texttt{-\/-as-cran}.
La vérification échoue si elle retourne une erreur, un avertissement ou une note.

Les packages supplémentaires, nécessaires aux actions de Travis en cas de succès, sont déclarés dans \texttt{r\_packages}:
...
La partie \texttt{after\_success} du script permet d'effectuer des tâches seulement dans cette situation: typiquement appeler Codecov pour mesurer le taux de couverture du code et utiliser \textbf{pkgdown} pour créer le site web d'aide:
...
La construction du site par \textbf{pkgdown} est plus compliqué que celle d'un livre produit par \textbf{bookdown} parce que deux versions coexistent si le package est hébergé sur CRAN: à la racine du dossier \texttt{docs}, la version de production correspondant à la version sur CRAN et, dans \texttt{docs/dev}, la version correspondant à la version en cours du package.
...
La dernière ligne du script consiste à créer un fichier vide nommé \texttt{.nojekyll} pour indiquer à GitHub que les pages web produites ne doivent pas être traitées comme le code source d'un site Jekyll, ce qui est le comportement par défaut des pages GitHub.

L'exemple suivant correspond au package \textbf{entropart}, déposé sur CRAN.
...
Pour des paramétrages particuliers, la documentation complète de Travis peut être consultée\footnote{\url{https://docs.travis-ci.com/user/languages/r/}}.

\hypertarget{vuxe9rification-de-.travis.yml}{%
\subsubsection{\texorpdfstring{Vérification de \texttt{.travis.yml}}{Vérification de .travis.yml}}\label{vuxe9rification-de-.travis.yml}}

Le débogage de la configuration de Travis est particulièrement laborieux parce que toute modification du fichier \texttt{.travis.yml} nécessite d'être validée et poussée pour déclencher Travis et éventuellement générer des erreurs.
L'ensemble du processus peut durer plusieurs dizaines de minutes.
Un outil de vérification en ligne\footnote{\url{https://config.travis-ci.com/explore}} permet de valider au moins la syntaxe du fichier et donc de gagner du temps.

\hypertarget{probluxe8mes-transitoires}{%
\subsubsection{Problèmes transitoires}\label{probluxe8mes-transitoires}}

Les changements de version de R, de LaTeX, de pandoc\ldots{} peuvent générer des erreurs dans l'intégration continue.
Une bonne pratique consiste à adapter son fichier \texttt{.travis.yml} en s'inspirant de projets qui fonctionnent: il suffit de regarder leur code sur GitHub.
...
\tightlist
\item
  pour les projets d'ouvrage, une version plus ancienne de pandoc sur Travis que sur le poste de travail empêche le bon tricot du document alors qu'il fonctionne parfaitement en local.
  La solution consiste à forcer la version de pandoc (elle devra être mise à jour régulièrement).
  Le début de fichier suivant force les versions de Linux et de pandoc:
\end{itemize}
...
\tightlist
\item
  pour tous les projets, la défaillance d'un package importé qui ne s'installe pas correctement peut être contournée en utilisant les versions binaires des packages fournies par RStudio.
  Les commandes suivantes, ajoutées avant les scripts, permettent de le faire:
\end{itemize}

\begin{verbatim}
# Packages binaires à partir de RStudio
before_install:
  - echo "options(repos = 
  ...
  - echo "options(HTTPUserAgent = paste0('R/',  |>
    ...
    R.version['os']), ')'))" >> ~/.Rprofile.site
\end{verbatim}

Ces contournements devront être réévalués régulièrement et supprimés quand ils ne seront plus utiles.

\hypertarget{ajouter-des-badges}{%
\subsection{Ajouter des badges}\label{ajouter-des-badges}}
...
[![Travis-CI Build Status](https://travis-ci.org/ |>
  ...
  (https://travis-ci.org/<GitHubID>/<Depot>)
\end{verbatim}
...
Sur le site de GitHub Classrooms\footnote{\url{https://classroom.github.com/}}, cliquer sur ``Sign in'' et utiliser son compte GitHub pour s'authentifier.
