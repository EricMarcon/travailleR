...
##  f1(X)   1.722   6.4165  11.28238   6.8470   7.8310 399.381
##  f2(X)   4.182   8.0360  12.71697   8.5075   9.3480 395.814
##  f3(X)  97.990 105.3495 113.44290 106.9280 108.9575 609.547
##  f4(X) 131.938 138.2930 152.24653 140.6710 142.8850 713.769
##  neval
##    100
##    100
##    100
...
##                                    expr   median
## 1                              sqrt(x1)   1.7425
## 2                lapply(x1, FUN = sqrt) 144.6890
## 3                sapply(x1, FUN = sqrt) 177.2840
## 4 vapply(x1, FUN = sqrt, FUN.VALUE = 0) 137.2885
\end{verbatim}
...
\begin{verbatim}
##          expr   median
## 1  Boucle(x1) 350.3040
## 2 Boucle2(x1)  42.2095
\end{verbatim}
...
\begin{verbatim}
##                        expr   median
## 1 vapply(x1, FUN = sqrt, 0) 132.8195
## 2                Boucle(x1)  41.7790
\end{verbatim}
...
\begin{verbatim}
##                                               expr   median
## 1                        vapply(x1, FUN = sqrt, 0) 134.5415
## 2 for (i in 1:length(x1)) Racine[i] <- sqrt(x1[i]) 994.9265
\end{verbatim}
...
\begin{verbatim}
##                        expr   median
## 1 replicate(1000, runif(1)) 839.7620
## 2               runif(1000)   6.6625
\end{verbatim}
...
\begin{verbatim}
##                              expr   median
## 1   BoucleSomme(diamonds[-(2:4)]) 1.548221
## 2 apply(diamonds[-(2:4)], 2, sum) 3.549124
## 3       colSums(diamonds[-(2:4)]) 1.235248
\end{verbatim}
...
La mise en place de la parallélisation a un coût d'environ \texttt{0.04} secondes ici.
...
##  298.931 305.6755 346.4447 311.6205 315.618 3043.594   100
\end{verbatim}
...
Ce temps est à comparer à celui de la boucle for de référence, multiplié par 2500, soit \texttt{1980} secondes.
...
## ● completed target NbPoints [0.77 seconds, 53 bytes]
## ▶ dispatched target X
## ● completed target X [0.003 seconds, 11.058 kilobytes]
## ▶ dispatched target d
## ● completed target d [0.033 seconds, 55 bytes]
## ▶ dispatched target map
## ● completed target map [0.012 seconds, 187.39 kilobytes]
## ▶ ended pipeline [0.905 seconds]
\end{verbatim}
...
\begin{verbatim}
## ▶ dispatched target NbPoints
## ● completed target NbPoints [0.657 seconds, 53 bytes]
## ▶ dispatched target X
## ● completed target X [0.001 seconds, 11.058 kilobytes]
## ▶ dispatched target d
## ● completed target d [0.036 seconds, 55 bytes]
## ▶ dispatched target map
## ● completed target map [0.012 seconds, 187.39 kilobytes]
## ▶ ended pipeline [0.792 seconds]
\end{verbatim}
...
##  triple.integer(2L)  82 123 6556.31    123 123 642716   100
##   triple.numeric(2) 123 123 8439.03    123 164 830127   100
##          triple(2L) 492 533  571.95    533 574   2870   100
\end{verbatim}
