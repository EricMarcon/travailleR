...
Dès la première utilisation de RStudio, créer un nouveau fichier R (menu ``File \textgreater{} New File \textgreater{} R Script''), l'enregistrer au format UTF8 (``File \textgreater{} Save with Encoding\ldots{}''), choisir UTF8 dans la liste des formats et cocher la case ``Set as default encoding for source files''.
Supprimer le fichier après l'avoir enregistré.

Les nouveaux fichiers seront codés au format UTF8.
Les fichiers codés sous un autre format ne s'afficheront pas correctement: ils pourront être réouverts avec leur codage d'origine (``File \textgreater{} Reopen with Encoding\ldots{}''), en essayant éventuellement plusieurs codages jusqu'à obtenir un affichage correct, et sauvegardés au format UTF8 ensuite.
...
Il faut systématiquement travailler dans des sous-dossiers de \texttt{\textasciitilde{}}, par exemple : \texttt{\textasciitilde{}\textbackslash{}Formation}.
...
Un bouton ``Install'' se trouve dans la fenêtre \emph{Packages} de RStudio.
...
Le bouton ``Update'' dans la fenêtre \emph{Packages} de RStudio permet de mettre à jour tous les packages installés.
...
\FunctionTok{InstallPackages}\NormalTok{(}\FunctionTok{c}\NormalTok{(}
  ...
  \CommentTok{\# Modèles de documents}
  \StringTok{"memoiR"}
\NormalTok{  ))}
...
Dans RStudio, modifier les options globales (menu ``Tools \textgreater{} Global Options\ldots{}'').
...
L'adresse de messagerie (qui remplace ``user.email'') doit être celle enregistrée dans la configuration de git et le compte GitHub.
...
Les jetons sont créés sur GitHub, dans les paramètres de son compte d'utilisateur, dans ``Developer Settings \textgreater{} Personal Access Tokens''\footnote{\url{https://help.github.com/en/github/authenticating-to-github/creating-a-personal-access-token-for-the-command-line}}.

Générer un nouveau jeton, le décrire en tant que ``git-RStudio'' et lui donner l'autorisation ``repo'', c'est-à-dire modifier \emph{tous} les dépôts (il n'est pas possible de limiter l'accès à un dépôt particulier).
...
Lier l'installation locale au compte: dans le menu ``Edition \textgreater{} Préférences'', sélectionner ``Synchronisation \textgreater{} Paramètres'' et s'authentifier dans la zone ``Synchronisation des données''.
...
Télécharger l'extension Better BibTeX\footnote{\url{https://retorque.re/zotero-better-bibtex/installation/}} et l'installer avec le menu ``Outils \textgreater{} Extensions'': cliquer sur le bouton des paramètres en haut à droite de la fenêtre, puis ``Install Add-on From File\ldots{}'' et sélectionner le fichier qui vient d'être téléchargé.

Paramétrer Better BibTeX à partir du menu ``Edition \textgreater{} Préférences \textgreater{} Better BibTeX''.
Les options à modifier sont les suivantes:

\begin{itemize}
\tightlist
\item
  ``Clés de citation \textgreater{} Format de clé'': \texttt{{[}auth:capitalize{]}{[}year{]}} pour que les citations disposent d'un identifiant unique de la forme ``Nom2021'';
\item
  ``Clés de citation \textgreater{} Conserver les clés de citation unique dans'': ``Toutes les collections'' pour que les identifiants des citations ne soient pas ambigus.
\item
  ``Exportation \textgreater{} Gestion des Champs \textgreater{} Champs à exclure de l'exportation'': ``abstract, file'' pour ne pas générer des fichiers bibliographiques surchargés d'informations inutiles dans les projets R.
\end{itemize}

Il est conseillé d'utiliser l'extension ZotFile\footnote{\url{http://zotfile.com/}} pour mieux contrôler l'emplacement du texte intégral (les fichiers PDF liés au références bibliographiques).
L'installer puis la paramétrer dans ``Outils \textgreater{} Préférences de ZotFile\ldots\textgreater{} Paramètres généraux \textgreater{} Emplacement des fichiers \textgreater{} Dossier personnalisé'': choisir le dossier de stockage des textes intégraux.
...
\NormalTok{PersonneR6 }\OtherTok{\textless{}{-}} \FunctionTok{R6Class}\NormalTok{(}\StringTok{"PersonneR6"}\NormalTok{, }\AttributeTok{public =} \FunctionTok{list}\NormalTok{(}\AttributeTok{Nom =} \StringTok{"character"}\NormalTok{,}
    \AttributeTok{Prenom =} \StringTok{"character"}\NormalTok{, }\AttributeTok{initialize =} \ControlFlowTok{function}\NormalTok{(}\AttributeTok{Nom =} \ConstantTok{NA}\NormalTok{, }\AttributeTok{Prenom =} \ConstantTok{NA}\NormalTok{) \{}
...
\NormalTok{    \}, }\AttributeTok{print =} \ControlFlowTok{function}\NormalTok{() }\FunctionTok{cat}\NormalTok{(self}\SpecialCharTok{$}\NormalTok{Prenom, self}\SpecialCharTok{$}\NormalTok{Nom)))}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Fonction (mal écrite) utilisant une variable qu\textquotesingle{}elle ne}
...
##  f1(X)  40.939  43.5520  59.65316  45.5060  48.7445
##  f2(X)  47.588  51.2930  67.11952  52.4020  54.9805
##  f3(X) 279.584 282.9865 308.89160 284.7130 291.7925
##  f4(X) 412.673 416.9155 450.17382 419.8965 429.0305
##       max neval
##  1325.719   100
##  1398.770   100
##  1799.519   100
...
\begin{center}\includegraphics[width=0.8\linewidth]{travailleR_files/figure-latex/boxplot-1} \end{center}
...
\begin{center}\includegraphics[width=0.8\linewidth]{travailleR_files/figure-latex/boxplot-2} \end{center}
...
##                                    expr   median
## 1                              sqrt(x1)   4.7745
## 2                lapply(x1, FUN = sqrt) 308.6985
## 3                sapply(x1, FUN = sqrt) 372.5685
## 4 vapply(x1, FUN = sqrt, FUN.VALUE = 0) 304.6100
\end{verbatim}
...
\begin{verbatim}
##          expr   median
## 1  Boucle(x1) 832.1550
## 2 Boucle2(x1)  77.1935
\end{verbatim}
...
\begin{verbatim}
##                        expr   median
## 1 vapply(x1, FUN = sqrt, 0) 291.9255
## 2                Boucle(x1)  76.0670
\end{verbatim}
...
\begin{verbatim}
##                        expr    median
## 1 replicate(1000, runif(1)) 3011.7740
## 2               runif(1000)   33.8005
\end{verbatim}
...
\begin{verbatim}
##                              expr   median
## 1   BoucleSomme(diamonds[-(2:4)]) 3.746955
## 2 apply(diamonds[-(2:4)], 2, sum) 9.679460
## 3       colSums(diamonds[-(2:4)]) 2.570198
\end{verbatim}
...
La mise en place de la parallélisation a un coût d'environ \texttt{0.14} secondes ici.
...
\NormalTok{Temps }\OtherTok{\textless{}{-}} \FunctionTok{sapply}\NormalTok{(Taches, }\ControlFlowTok{function}\NormalTok{(nbTaches) \{}
...
\NormalTok{  ggplot }\SpecialCharTok{+}
  ...
  \FunctionTok{geom\_vline}\NormalTok{(}\AttributeTok{xintercept =}\NormalTok{ nbCoeurs, }\AttributeTok{col =} \StringTok{"red"}\NormalTok{, }\AttributeTok{lty =} \DecValTok{2}\NormalTok{) }\SpecialCharTok{+}
  \FunctionTok{geom\_vline}\NormalTok{(}\AttributeTok{xintercept =} \DecValTok{2} \SpecialCharTok{*}\NormalTok{ nbCoeurs, }\AttributeTok{col =} \StringTok{"red"}\NormalTok{, }\AttributeTok{lty =} \DecValTok{2}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{center}\includegraphics[width=0.8\linewidth]{travailleR_files/figure-latex/pairdist-1} \end{center}
...
\begin{center}\includegraphics[width=0.8\linewidth]{travailleR_files/figure-latex/RCpp-1} \end{center}
...
##  2.074905 2.129105 2.323001 2.171333 2.214298 12.50891
##  neval
##    100
\end{verbatim}
...
\begin{center}\includegraphics[width=0.8\linewidth]{travailleR_files/figure-latex/RcppParallel-1} \end{center}
...
Ce temps est à comparer à celui de la boucle for de référence, multiplié par 2500, soit \texttt{6327} secondes.
...
\begin{center}\includegraphics[width=0.8\linewidth]{travailleR_files/figure-latex/tar_visnetwork-1} \end{center}
...
\begin{center}\includegraphics[width=0.8\linewidth]{travailleR_files/figure-latex/tar_visnetwork2-1} \end{center}
...
\begin{center}\includegraphics[width=0.8\linewidth]{travailleR_files/figure-latex/tar_read-1} \end{center}
...
Les fichiers pris en compte peuvent être validés (\emph{Commit}) en cliquant sur le bouton ``Commit'' dans la fenêtre \emph{Git}.
...
Par la suite, il faudra cliquer sur le bouton ``Push'' de la fenêtre \emph{Git} pour le faire.
...
Toute séance de travail sur un projet commence en tirant (Bouton ``Pull'') de la fenêtre \emph{Git} pour intégrer au dépôt local les mises à jour effectuées sur GitHub par d'autres collaborateurs.
...
Cliquer sur le bouton violet ``New Branch'' dans la fenêtre \emph{git} de RStudio.
...
Les premières modifications de la nouvelle branche doivent être poussées en ligne de commande parce que les boutons ``Push'' et ``Pull'' de la fenêtre \emph{Git} ne fonctionnent pas tant que la branche n'existe pas sur le dépôt distant.
...
\hypertarget{guxe9nuxe9ration-dune-paire-de-cluxe9s-pour-le-propriuxe9taire-du-projet}{%
...
Stocker la clé publique sur GitHub dans ``Settings \textgreater{} SSH and GPG Keys''.
...
\hypertarget{guxe9nuxe9ration-dune-paire-de-cluxe9s-pour-le-projet}{%
...
\tightlist
\item
  Entrer le nom de la clé: \texttt{NomDuProjet.rsa}.
\item
  Ne pas saisir de phrase de validation (mot de passe) pour permettre l'utilisation de la clé sans interaction.
\end{itemize}

La clé privée \texttt{NomDuProjet.rsa} ne doit être diffusée qu'aux ayant-droits du projet.
...
Appliquer la procédure de la section \ref{sec:secrets-ci} pour créer un secret nommé ``RSA'' et coller le contenu du fichier \texttt{NomDuProjet.rsa} dans le champ ``Value'' du formulaire.

L'utilisation du secret est décrite dans la section \ref{sec:confidentielCI}.

\hypertarget{cruxe9ation-dun-coffre-fort}{%
...
\hypertarget{ajout-des-utilisateurs}{%
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Liste des propriétaire de la donnée \textquotesingle{}X\textquotesingle{}}
...
\begin{verbatim}
## v Setting active project to '/Users/runner/work/travailleR/
## travailleR'
\end{verbatim}

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Lecture de la donnée \textquotesingle{}X\textquotesingle{}}
...
Le code R est inclus dans des bouts de code (\emph{code chunks}) créés facilement en cliquant sur le bouton ``Insert a new code chunk'' au-dessus de la fenêtre du code source dans RStudio.
...
Les options par défaut sont déclarées dans le bout de code nommé ``Options'' au début du document Markdown, dans la fonction \texttt{opts\_chunk\$set()}.
...
Le texte de \texttt{fig.cap}, ``Titre de la figure'' précédemment, est remplacé par \texttt{(ref:pressure)} \emph{à l'intérieur des guillemets} qui sont conservés et la légende est saisie dans un paragraphe commençant par \texttt{(ref:pressure)} suivi d'un espace.
...
\FunctionTok{names}\NormalTok{(mes\_iris) }\OtherTok{\textless{}{-}} \FunctionTok{c}\NormalTok{(}\StringTok{"Longueur sépales ($l\_s$)"}\NormalTok{, }\StringTok{"Largeur"}\NormalTok{,}
    \StringTok{"Longueur pétales"}\NormalTok{, }\StringTok{"Largeur"}\NormalTok{, }\StringTok{"Espèce"}\NormalTok{)}
\NormalTok{knitr}\SpecialCharTok{::}\FunctionTok{kable}\NormalTok{(mes\_iris, }\AttributeTok{caption =} \StringTok{"Tableau créé par kable"}\NormalTok{,}
...
Utiliser ensuite le menu contextuel ``Exporter la collection\ldots{}'' et sélectionner:
...
\item
  Cliquer sur ``OK'' puis choisir le nom du fichier (\texttt{references.bib}) et son emplacement (le dossier du projet R).
\end{itemize}
...
Sélectionner le modèle \emph{Simple Article} du package \textbf{memoiR}, choisir le nom du projet (``Name:'', qui sera le nom du dossier dans lequel il sera créé, et son dossier parent (``Location:'').
...
Quand un dossier est créé, par exemple par le modèle \emph{Simple Article}, il faut en faire un projet RStudio: dans le menu des projets (en haut à droite de la fenêtre de RStudio), utiliser le menu ``New Project\ldots{}'' puis ``Existing Directory'' et sélectionner le dossier qui vient d'être créé.
...
Le livre entier est créé en cliquant sur le bouton ``Build Book'' de la fenêtre \emph{Build} de RStudio.
...
Dans RStudio, dans le menu des projets en haut à droite, cliquer sur ``New Project\ldots{}'' puis ``New Directory'' puis ``Simple R Markdown website''.
...
Un site Hugo peut être hébergé par exemple sur la page personnelle de tout utilisateur de GitHub dont l'adresse est de la forme ``GitHubID.github.io''.
...
Sur la page du dépôt \emph{starter-academic}\footnote{\url{https://github.com/wowchemy/starter-academic}}, cliquer sur le bouton ``Use this template'', s'authentifier éventuellement sur GitHub, puis saisir le nom du dépôt qui contiendra le projet, par exemple ``MySite''.
...
Dans RStudio, créer un nouveau projet à partir de GitHub: dans le menu des projets en haut à droite, cliquer sur ``New Project\ldots{}'' puis ``Version Control'' puis ``Git'' puis coller l'adresse dans le champ ``Repository URL'' (figure \ref{fig:rediger-Projet-GitHub}).
...
Enfin, \textbf{ragg} peut être utilisé comme moteur de rendu graphique par défaut dans RStudio à partir de la version 1.4 (Menu ``Tools \textgreater{} Global Options \textgreater{} General \textgreater{} Graphics \textgreater{} Backend'').
...
\begin{center}\includegraphics[width=0.8\linewidth]{travailleR_files/figure-latex/tar_visnetwork_4-1} \end{center}
...
\begin{center}\includegraphics[width=0.8\linewidth]{travailleR_files/figure-latex/tar_read_4-1} \end{center}
...
\tightlist
\item
  Construire le fichier source du package (``Build \textgreater{} More \textgreater{} Build Source Package'') puis l'installer (``Packages \textgreater{} Install \textgreater{} Install from \textgreater{} Package Archive file'').
...
## Unit: nanoseconds
##                expr  min   lq     mean median     uq
##  triple.integer(2L)  336  346 32859.51  361.0  386.5
##   triple.numeric(2)  335  348 28442.99  363.0  397.0
##          triple(2L) 1351 1377  1574.29 1404.5 1473.0
##      max neval
##  3248138   100
##  2804556   100
##     7970   100
...
\begin{center}\includegraphics[width=0.8\linewidth]{travailleR_files/figure-latex/plot.multiple-1} \end{center}
...
\begin{center}\includegraphics[width=0.8\linewidth]{travailleR_files/figure-latex/plot.multiple-2} \end{center}
...
Sur le site \emph{R-hub builder}, cliquer sur ``Advanced'', sélectionner le fichier source du package et la plateforme de test: \emph{Debian Linux, R-devel, GCC}.
...
\hypertarget{obtention-dun-jeton-daccuxe8s-personnel}{%
...
\hypertarget{internationalisation}{%
...
L'étape ``Upload artifact'' est modifiée pour s'exécuter malgré l'échec de l'étape précédente en ajoutant la ligne \texttt{if:}:
...
Une étape est ajoutée avant ``Upload artifact'' pour copier le résultat du tricot et le fichier \texttt{.log}:
...
Cliquer sur le bouton ``\ldots{}'' puis sur ``Create Status Badge''.
...
Sélectionner le dépôt modèle (\emph{starter code}) puis cliquer sur ``Continue'' puis ``Create Assignment''.
