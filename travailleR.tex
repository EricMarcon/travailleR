...
Lier l'installation locale au compte: dans le menu \enquote{Edition \textgreater{} Préférences}, sélectionner \enquote{Synchronisation \textgreater{} Paramètres} et s'authentifier dans la zone \enquote{Synchronisation des données}.
...
Paramétrer Better BibTeX à partir du menu \enquote{Edition \textgreater{} Préférences \textgreater{} Better BibTeX}.
...
\item
  \enquote{Clés de citation \textgreater{} Conserver les clés de citation unique dans}: \enquote{Toutes les collections} pour que les identifiants des citations ne soient pas ambigus.
\item
...
L'installer puis la paramétrer dans \enquote{Outils \textgreater{} Préférences de ZotFile\ldots\textgreater{} Paramètres généraux \textgreater{} Emplacement des fichiers \textgreater{} Dossier personnalisé}: choisir le dossier de stockage des textes intégraux.
...
\NormalTok{PersonneR6 <-}\StringTok{ }\KeywordTok{R6Class}\NormalTok{(}\StringTok{"PersonneR6"}\NormalTok{, }\DataTypeTok{public =} \KeywordTok{list}\NormalTok{(}\DataTypeTok{Nom =} \StringTok{"character"}\NormalTok{,}
    \DataTypeTok{Prenom =} \StringTok{"character"}\NormalTok{, }\DataTypeTok{initialize =} \ControlFlowTok{function}\NormalTok{(}\DataTypeTok{Nom =} \OtherTok{NA}\NormalTok{, }\DataTypeTok{Prenom =} \OtherTok{NA}\NormalTok{) \{}
...
##  f1(X)  37.560  39.4735  56.81346  40.8800  45.4745
##  f2(X)  45.188  47.1640  66.90455  48.4005  51.9940
##  f3(X) 277.627 279.9785 322.16932 283.8930 292.2080
##  f4(X) 407.147 410.2940 460.26005 417.9145 430.6465
##       max neval
##  1388.380   100
##  1568.827   100
##  1935.704   100
...
##                                    expr   median
## 1                              sqrt(x1)   4.7375
## 2                lapply(x1, FUN = sqrt) 313.4175
## 3                sapply(x1, FUN = sqrt) 376.5980
## 4 vapply(x1, FUN = sqrt, FUN.VALUE = 0) 313.9650
\end{verbatim}
...
\normalsize
Le gain est considérable: de 1 à \texttt{12}.
...
\begin{verbatim}
##                        expr   median
## 1 vapply(x1, FUN = sqrt, 0) 305.3245
## 2                Boucle(x1)  76.4020
\end{verbatim}
...
\begin{verbatim}
##                                               expr   median
## 1                        vapply(x1, FUN = sqrt, 0)  310.387
## 2 for (i in 1:length(x1)) Racine[i] <- sqrt(x1[i]) 3260.521
\end{verbatim}
...
\begin{verbatim}
##                        expr    median
## 1 replicate(1000, runif(1)) 3868.4530
## 2               runif(1000)   33.9185
\end{verbatim}
...
\begin{verbatim}
##                              expr   median
## 1   BoucleSomme(diamonds[-(2:4)]) 3.679657
## 2 apply(diamonds[-(2:4)], 2, sum) 9.372713
## 3       colSums(diamonds[-(2:4)]) 2.479086
\end{verbatim}
...
\CommentTok{# Série}
\KeywordTok{system.time}\NormalTok{(}\KeywordTok{foreach}\NormalTok{(}\DataTypeTok{i =} \KeywordTok{icount}\NormalTok{(nbCoeurs), }\DataTypeTok{.combine =} \StringTok{"c"}\NormalTok{) }\OperatorTok{%do%}
\StringTok{    }\NormalTok{\{}
        \KeywordTok{f}\NormalTok{(i)}
\NormalTok{    \})}
\end{Highlighting}
...
\begin{Highlighting}[]
\CommentTok{# Parallèle}
\KeywordTok{system.time}\NormalTok{(}\KeywordTok{foreach}\NormalTok{(}\DataTypeTok{i =} \KeywordTok{icount}\NormalTok{(nbCoeurs), }\DataTypeTok{.combine =} \StringTok{"c"}\NormalTok{) }\OperatorTok{%dopar%}
\StringTok{    }\NormalTok{\{}
        \KeywordTok{f}\NormalTok{(i)}
\NormalTok{    \})}
\end{Highlighting}
...
\NormalTok{    distances <-}\StringTok{ }\KeywordTok{sapply}\NormalTok{(}\DecValTok{1}\OperatorTok{:}\NormalTok{NbPoints, }\ControlFlowTok{function}\NormalTok{(i) }\KeywordTok{sapply}\NormalTok{(}\DecValTok{1}\OperatorTok{:}\NormalTok{NbPoints,}
        \ControlFlowTok{function}\NormalTok{(j) }\KeywordTok{sqrt}\NormalTok{((X}\OperatorTok{$}\NormalTok{x[i] }\OperatorTok{-}\StringTok{ }\NormalTok{X}\OperatorTok{$}\NormalTok{x[j])}\OperatorTok{^}\DecValTok{2} \OperatorTok{+}\StringTok{ }\NormalTok{(X}\OperatorTok{$}\NormalTok{y[i] }\OperatorTok{-}\StringTok{ }\NormalTok{X}\OperatorTok{$}\NormalTok{y[j])}\OperatorTok{^}\DecValTok{2}\NormalTok{)))}
...
\NormalTok{    \}, }\DecValTok{1}\OperatorTok{:}\DecValTok{1000} \OperatorTok{+}\StringTok{ }\DecValTok{0}\NormalTok{)}
    \KeywordTok{return}\NormalTok{(}\KeywordTok{sum}\NormalTok{(}\KeywordTok{sqrt}\NormalTok{(distances))}\OperatorTok{/}\NormalTok{NbPoints}\OperatorTok{/}\NormalTok{(NbPoints }\OperatorTok{-}\StringTok{ }\DecValTok{1}\NormalTok{) }\OperatorTok{*}\StringTok{ }\DecValTok{2}\NormalTok{)}
\NormalTok{\}}
...
\NormalTok{        \}, }\DecValTok{0}\NormalTok{)}
\NormalTok{    \})}
    \KeywordTok{return}\NormalTok{(}\KeywordTok{sum}\NormalTok{(}\KeywordTok{sqrt}\NormalTok{(}\KeywordTok{simplify2array}\NormalTok{(distances)))}\OperatorTok{/}\NormalTok{NbPoints}\OperatorTok{/}\NormalTok{(NbPoints }\OperatorTok{-}
...
\NormalTok{            distance <-}\StringTok{ }\NormalTok{distance }\OperatorTok{+}\StringTok{ }\KeywordTok{sqrt}\NormalTok{((X}\OperatorTok{$}\NormalTok{x[i] }\OperatorTok{-}\StringTok{ }\NormalTok{X}\OperatorTok{$}\NormalTok{x[j])}\OperatorTok{^}\DecValTok{2} \OperatorTok{+}
...
\NormalTok{    distances <-}\StringTok{ }\KeywordTok{foreach}\NormalTok{(}\DataTypeTok{i =} \DecValTok{1}\OperatorTok{:}\NormalTok{NbPointsReduit, }\DataTypeTok{.combine =} \StringTok{"cbind"}\NormalTok{) }\OperatorTok{%:%}
\StringTok{        }\KeywordTok{foreach}\NormalTok{(}\DataTypeTok{j =} \DecValTok{1}\OperatorTok{:}\NormalTok{NbPointsReduit, }\DataTypeTok{.combine =} \StringTok{"c"}\NormalTok{) }\OperatorTok{%do%}\StringTok{ }\NormalTok{\{}
        \ControlFlowTok{if}\NormalTok{ (j }\OperatorTok{>}\StringTok{ }\NormalTok{i) \{}
...
\NormalTok{        \}}
\NormalTok{    \}}
    \KeywordTok{return}\NormalTok{(}\KeywordTok{sum}\NormalTok{(}\KeywordTok{sqrt}\NormalTok{(distances))}\OperatorTok{/}\NormalTok{NbPointsReduit}\OperatorTok{/}\NormalTok{(NbPointsReduit }\OperatorTok{-}
\StringTok{        }\DecValTok{1}\NormalTok{) }\OperatorTok{*}\StringTok{ }\DecValTok{2}\NormalTok{)}
\NormalTok{\}}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{(mb <-}\StringTok{ }\KeywordTok{microbenchmark}\NormalTok{(d <-}\StringTok{ }\KeywordTok{TotalDistance}\NormalTok{(X}\OperatorTok{$}\NormalTok{x, X}\OperatorTok{$}\NormalTok{y)}\OperatorTok{/}\NormalTok{NbPoints}\OperatorTok{/}\NormalTok{(NbPoints }\OperatorTok{-}
\StringTok{    }\DecValTok{1}\NormalTok{) }\OperatorTok{*}\StringTok{ }\DecValTok{2}\NormalTok{))}
\end{Highlighting}
\end{Shaded}
...
##  789.221 930.0485 2077.189 2110.001 2231.706 23721.99   100
\end{verbatim}
...
Ce temps est à comparer à celui de la boucle for de référence, multiplié par 2500, soit \texttt{5922} secondes.

\scriptsize

\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{system.time}\NormalTok{(d <-}\StringTok{ }\KeywordTok{TotalDistance}\NormalTok{(X}\OperatorTok{$}\NormalTok{x, X}\OperatorTok{$}\NormalTok{y)}\OperatorTok{/}\NormalTok{NbPoints}\OperatorTok{/}\NormalTok{(NbPoints }\OperatorTok{-}
\StringTok{    }\DecValTok{1}\NormalTok{) }\OperatorTok{*}\StringTok{ }\DecValTok{2}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Highlighting}[]
\KeywordTok{library}\NormalTok{(}\StringTok{"tidyverse"}\NormalTok{)}
\NormalTok{mes_iris <-}\StringTok{ }\KeywordTok{head}\NormalTok{(iris)}
\KeywordTok{names}\NormalTok{(mes_iris) <-}\StringTok{ }\KeywordTok{c}\NormalTok{(}\StringTok{"Longueur sépales ($l_s$)"}\NormalTok{, }\StringTok{"Largeur"}\NormalTok{,}
    \StringTok{"Longueur pétales"}\NormalTok{, }\StringTok{"Largeur"}\NormalTok{, }\StringTok{"Espèce"}\NormalTok{)}
\NormalTok{knitr}\OperatorTok{::}\KeywordTok{kable}\NormalTok{(mes_iris, }\DataTypeTok{caption =} \StringTok{"Tableau créé par kable"}\NormalTok{,}
    \DataTypeTok{booktabs =} \OtherTok{TRUE}\NormalTok{, }\DataTypeTok{escape =} \OtherTok{FALSE}\NormalTok{) }\OperatorTok{%>%}
...
\StringTok{    }\CommentTok{# Premières lignes}
\KeywordTok{head}\NormalTok{() }\OperatorTok{%>%}
\StringTok{    }\CommentTok{# Création d'un objet flextable}
\KeywordTok{flextable}\NormalTok{() }\OperatorTok{%>%}
\StringTok{    }\CommentTok{# Titre des colonnes}
\KeywordTok{set_header_labels}\NormalTok{(}\DataTypeTok{Sepal.Length =} \StringTok{"Longueur sépales"}\NormalTok{, }\DataTypeTok{Sepal.Width =} \StringTok{"Largeur"}\NormalTok{,}
    \DataTypeTok{Petal.Length =} \StringTok{"Longueur pétales"}\NormalTok{, }\DataTypeTok{Petal.Width =} \StringTok{"Largeur"}\NormalTok{,}
    \DataTypeTok{Species =} \StringTok{"Espèce"}\NormalTok{) }\OperatorTok{%>%}
\StringTok{    }\CommentTok{# Sélection des longs sépales (>5) et affichage en}
\StringTok{    }\CommentTok{# rouge}
\KeywordTok{color}\NormalTok{(}\OperatorTok{~}\NormalTok{Sepal.Length }\OperatorTok{>}\StringTok{ }\DecValTok{5}\NormalTok{, }\OperatorTok{~}\NormalTok{Sepal.Length, }\DataTypeTok{color =} \StringTok{"red"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Highlighting}[]
\NormalTok{knitr}\OperatorTok{::}\NormalTok{opts_chunk}\OperatorTok{$}\KeywordTok{set}\NormalTok{(}
  \DataTypeTok{cache=}\OtherTok{FALSE}\NormalTok{, }\CommentTok{# Cache chunk results}
  \DataTypeTok{echo =} \OtherTok{TRUE}\NormalTok{, }\CommentTok{# Show/Hide R chunks}
  \DataTypeTok{warning=}\OtherTok{FALSE}\NormalTok{, }\CommentTok{# Show/Hide warnings}
  \CommentTok{# Figure alignment and size}
  \DataTypeTok{fig.align=}\StringTok{'center'}\NormalTok{, }\DataTypeTok{out.width=}\StringTok{'80%'}\NormalTok{, }\DataTypeTok{fig.asp=}\NormalTok{.}\DecValTok{75}\NormalTok{,}
  \CommentTok{# Graphic devices (ragg_png is better than standard png)}
  \DataTypeTok{dev =} \KeywordTok{c}\NormalTok{(}\StringTok{"ragg_png"}\NormalTok{, }\StringTok{"pdf"}\NormalTok{),}
  \CommentTok{# Code chunk format}
  \DataTypeTok{tidy=}\OtherTok{TRUE}\NormalTok{, }\DataTypeTok{tidy.opts=}\KeywordTok{list}\NormalTok{(}\DataTypeTok{blank=}\OtherTok{FALSE}\NormalTok{, }\DataTypeTok{width.cutoff=}\DecValTok{60}\NormalTok{),}
  \DataTypeTok{size=}\StringTok{"scriptsize"}\NormalTok{, }\DataTypeTok{knitr.graphics.auto_pdf =} \OtherTok{TRUE}
\NormalTok{  )}
...
##  triple.integer(2L)  324  338 25392.25  361.5  388 2502236
##   triple.numeric(2)  321  338 29363.74  363.0  402 2897269
##          triple(2L) 1318 1356  1847.92 1388.0 1447   32765
##  neval
##    100
##    100
##    100
