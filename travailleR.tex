...
##  f1(X)  36.853  39.0395  60.25762  40.7715  45.3175
##  f2(X)  44.595  46.7030  68.70607  48.0910  53.3310
##  f3(X) 276.396 278.1820 312.09502 282.2040 301.1260
##  f4(X) 409.152 411.8890 448.55593 418.8350 436.1610
##       max neval
##  1727.472   100
##  1572.674   100
##  1711.902   100
...
##                                    expr   median
## 1                              sqrt(x1)   4.8900
## 2                lapply(x1, FUN = sqrt) 297.5860
## 3                sapply(x1, FUN = sqrt) 355.3110
## 4 vapply(x1, FUN = sqrt, FUN.VALUE = 0) 309.2525
\end{verbatim}
...
\begin{verbatim}
##                        expr   median
## 1 vapply(x1, FUN = sqrt, 0) 305.5835
## 2                Boucle(x1)  79.5615
\end{verbatim}
...
\begin{verbatim}
##                                               expr   median
## 1                        vapply(x1, FUN = sqrt, 0)  323.638
## 2 for (i in 1:length(x1)) Racine[i] <- sqrt(x1[i]) 3864.364
\end{verbatim}
...
\begin{verbatim}
##                        expr    median
## 1 replicate(1000, runif(1)) 4345.1260
## 2               runif(1000)   34.6765
\end{verbatim}
...
\begin{verbatim}
##                              expr   median
## 1   BoucleSomme(diamonds[-(2:4)]) 3.661311
## 2 apply(diamonds[-(2:4)], 2, sum) 9.460688
## 3       colSums(diamonds[-(2:4)]) 2.480700
\end{verbatim}
...
La mise en place de la parallélisation a un coût d'environ \texttt{0.014} secondes ici.
...
##  804.962 874.9435 1459.118 942.2875 2086.764 3925.511   100
\end{verbatim}
...
Ce temps est à comparer à celui de la boucle for de référence, multiplié par 2500, soit \texttt{4834} secondes.
...
\begin{Highlighting}[]
\KeywordTok{library}\NormalTok{(}\StringTok{"tidyverse"}\NormalTok{)}
\KeywordTok{names}\NormalTok{(iris) <-}\StringTok{ }\KeywordTok{c}\NormalTok{(}\StringTok{"Longueur sépales ($l_s$)"}\NormalTok{, }\StringTok{"Largeur"}\NormalTok{, }\StringTok{"Longueur pétales"}\NormalTok{,}
    \StringTok{"Largeur"}\NormalTok{, }\StringTok{"Espèce"}\NormalTok{)}
\NormalTok{knitr}\OperatorTok{::}\KeywordTok{kable}\NormalTok{(}\KeywordTok{head}\NormalTok{(iris), }\DataTypeTok{caption =} \StringTok{"Tableau créé par R"}\NormalTok{, }\DataTypeTok{booktabs =} \OtherTok{TRUE}\NormalTok{,}
...
La formule mathématique \(l_s\) a été retirée du titre de la colonne parce que \textbf{flextable} ne l'interprète pas de façon simple (mais une solution existe\footnote{\url{https://ardata-fr.github.io/flextable-gallery/gallery/}}).
...
Pour un usage occasionnel, \textbf{flextable} est plus difficile à prendre en main que \textbf{kableExtra} mais il est plus puissant si un usage intensif des tableaux est nécessaire.
...
## Unit: nanoseconds
##                expr  min     lq     mean median     uq
##  triple.integer(2L)  320  332.5 21801.76  340.5  363.5
##   triple.numeric(2)  318  333.0 19084.46  347.0  372.0
##          triple(2L) 1288 1331.5  1471.97 1367.5 1446.0
##      max neval
##  2144513   100
##  1872461   100
##     5918   100
