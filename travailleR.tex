...
##  f1(X)  36.072  37.9030  52.27757  38.9120  41.047 1264.824   100
##  f2(X)  45.662  47.2230  62.72502  48.3445  50.720 1307.774   100
##  f3(X) 262.280 269.7130 284.23920 271.5030 274.406 1527.832   100
##  f4(X) 389.072 401.4435 421.49348 402.7810 408.972 1885.247   100
\end{verbatim}
...
##                                    expr   median
## 1                              sqrt(x1)   4.8450
## 2                lapply(x1, FUN = sqrt) 331.9855
## 3                sapply(x1, FUN = sqrt) 408.7530
## 4 vapply(x1, FUN = sqrt, FUN.VALUE = 0) 327.2605
\end{verbatim}
...
\begin{verbatim}
##          expr   median
## 1  Boucle(x1) 844.9275
## 2 Boucle2(x1)  83.4340
\end{verbatim}
...
\begin{verbatim}
##                        expr  median
## 1 vapply(x1, FUN = sqrt, 0) 311.097
## 2                Boucle(x1)  81.452
\end{verbatim}
...
\begin{verbatim}
##                                               expr    median
## 1                        vapply(x1, FUN = sqrt, 0)  318.4645
## 2 for (i in 1:length(x1)) Racine[i] <- sqrt(x1[i]) 3602.6240
\end{verbatim}
...
\begin{verbatim}
##                        expr    median
## 1 replicate(1000, runif(1)) 3271.2140
## 2               runif(1000)   34.1145
\end{verbatim}
...
\begin{verbatim}
##                              expr   median
## 1   BoucleSomme(diamonds[-(2:4)]) 3.351122
## 2 apply(diamonds[-(2:4)], 2, sum) 8.267995
## 3       colSums(diamonds[-(2:4)]) 2.569251
\end{verbatim}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{Taches <-}\StringTok{ }\DecValTok{1}\OperatorTok{:}\NormalTok{(}\DecValTok{2} \OperatorTok{*}\StringTok{ }\NormalTok{nbCoeurs }\OperatorTok{+}\StringTok{ }\DecValTok{1}\NormalTok{)}
\NormalTok{Temps <-}\StringTok{ }\KeywordTok{sapply}\NormalTok{(Taches, }\ControlFlowTok{function}\NormalTok{(nbTaches) }\KeywordTok{system.time}\NormalTok{(}\KeywordTok{mclapply}\NormalTok{(}\DecValTok{1}\OperatorTok{:}\NormalTok{nbTaches, }
\NormalTok{    f, }\DataTypeTok{time =} \DecValTok{1}\NormalTok{, }\DataTypeTok{mc.cores =}\NormalTok{ nbCoeurs)))}
\KeywordTok{library}\NormalTok{(}\StringTok{"tidyverse"}\NormalTok{)}
\KeywordTok{tibble}\NormalTok{(Taches, }\DataTypeTok{Temps =}\NormalTok{ Temps[}\StringTok{"elapsed"}\NormalTok{, ]) }\OperatorTok{%>%}\StringTok{ }\NormalTok{ggplot }\OperatorTok{+}\StringTok{ }
\StringTok{    }\KeywordTok{geom_line}\NormalTok{(}\KeywordTok{aes}\NormalTok{(}\DataTypeTok{x =}\NormalTok{ Taches, }\DataTypeTok{y =}\NormalTok{ Temps)) }\OperatorTok{+}\StringTok{ }\KeywordTok{geom_vline}\NormalTok{(}\DataTypeTok{xintercept =}\NormalTok{ nbCoeurs, }
    \DataTypeTok{col =} \StringTok{"red"}\NormalTok{, }\DataTypeTok{lty =} \DecValTok{2}\NormalTok{) }\OperatorTok{+}\StringTok{ }\KeywordTok{geom_vline}\NormalTok{(}\DataTypeTok{xintercept =} \DecValTok{2} \OperatorTok{*}\StringTok{ }
\StringTok{    }\NormalTok{nbCoeurs, }\DataTypeTok{col =} \StringTok{"red"}\NormalTok{, }\DataTypeTok{lty =} \DecValTok{2}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\caption{Temps d'exécution en parallèle de tâches nécessitant une seconde (chaque tâche est une pause d'un quart de seconde). Le nombre de tâches varie de 1 à deux fois le nombre de coeurs utilisés (égal à \texttt{2}) plus une.}\label{fig:r-parallele}
\end{SCfigure}
...
## Unit: microseconds
##                                                      expr     min       lq
##  d <- TotalDistance(X$x, X$y)/NbPoints/(NbPoints - 1) * 2 667.707 673.2125
##      mean  median      uq      max neval
##  779.7484 714.416 806.764 2406.199   100
\end{verbatim}
...
## Unit: nanoseconds
##                expr  min     lq     mean median     uq     max neval
##  triple.integer(2L)  311  321.0   380.35    334  365.0    1491   100
##   triple.numeric(2)  313  320.5 21317.90    330  361.5 2095588   100
##          triple(2L) 1354 1386.5 22745.22   1412 1470.5 2122562   100
\end{verbatim}
