...
Le nom du package est entré sous la forme ``GitHubID/\break NomduPackage''.
...
##  f1(X)  36.207  38.8085  59.08418  42.7830  61.0085 332.983   100
##  f2(X)  44.134  48.2675  67.64770  55.2410  70.7325 347.272   100
##  f3(X) 269.322 286.5445 314.55758 293.8850 310.8080 759.272   100
##  f4(X) 401.267 427.2370 473.67168 439.7985 459.5405 837.748   100
\end{verbatim}
...
##                                    expr   median
## 1                              sqrt(x1)   4.6885
## 2                lapply(x1, FUN = sqrt) 317.9505
## 3                sapply(x1, FUN = sqrt) 374.3925
## 4 vapply(x1, FUN = sqrt, FUN.VALUE = 0) 312.3150
\end{verbatim}
...
\begin{verbatim}
##          expr   median
## 1  Boucle(x1) 819.0355
## 2 Boucle2(x1)  86.2060
\end{verbatim}
...
\begin{verbatim}
##                        expr   median
## 1 vapply(x1, FUN = sqrt, 0) 307.9715
## 2                Boucle(x1)  85.5590
\end{verbatim}
...
\begin{verbatim}
##                                               expr   median
## 1                        vapply(x1, FUN = sqrt, 0) 320.3190
## 2 for (i in 1:length(x1)) Racine[i] <- sqrt(x1[i]) 901.0185
\end{verbatim}
...
\begin{verbatim}
##                        expr    median
## 1 replicate(1000, runif(1)) 4060.4595
## 2               runif(1000)   34.8625
\end{verbatim}
...
\begin{verbatim}
##                              expr   median
## 1   BoucleSomme(diamonds[-(2:4)]) 3.848164
## 2 apply(diamonds[-(2:4)], 2, sum) 9.646429
## 3       colSums(diamonds[-(2:4)]) 2.521611
\end{verbatim}
...
## Unit: microseconds
##                                                      expr     min       lq
##  d <- TotalDistance(X$x, X$y)/NbPoints/(NbPoints - 1) * 2 667.474 674.6095
##      mean   median       uq      max neval
##  699.0063 679.6265 686.7545 2381.097   100
\end{verbatim}
...
Ce temps est à comparer à celui de la boucle for de référence, multiplié par 2500, soit \texttt{8434} secondes.
...
Elles sont affichées sur la page d'accueil de tout dépôt vide nouvellement créé sur GitHub et peuvent donc être copiées et coller directement vers le terminal.

\begin{verbatim}
git remote add origin https://github.com/GitHubID/NomduPackage.git
git branch -M master
...
\NormalTok{knitr}\OperatorTok{::}\KeywordTok{kable}\NormalTok{(}\KeywordTok{head}\NormalTok{(iris), }\DataTypeTok{caption =} \StringTok{"Tableau créé par R"}\NormalTok{, }
    \DataTypeTok{longtable =} \OtherTok{TRUE}\NormalTok{, }\DataTypeTok{booktabs =} \OtherTok{TRUE}\NormalTok{, }\DataTypeTok{escape =} \OtherTok{FALSE}\NormalTok{) }\OperatorTok{%>%}\StringTok{ }
...
## Unit: nanoseconds
##                expr  min     lq    mean median     uq   max neval
##  triple.integer(2L)  549  567.5  610.52  577.0  602.0  1123   100
##   triple.numeric(2)  547  567.0  609.42  578.0  592.5  2114   100
##          triple(2L) 1658 1686.5 2051.04 1705.5 1775.0 30132   100
\end{verbatim}
...
\begin{verbatim}
      - name: Render book
        run: |
          bookdown::render_book("index.Rmd", "bookdown::pdf_book", envir = new.env())
          bookdown::render_book("index.Rmd", "bookdown::gitbook", envir = new.env())
\end{verbatim}
