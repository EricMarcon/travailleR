...
Leur entête minimal est `{targets}` au lieu de est `{r}`, et ils doivent être nommés.
...
```{targets targets_global, tar_globals=TRUE, echo=FALSE, tar_interactive = FALSE}
tar_option_set(packages = c("spatstat", "dbmss"))
```

Les fonctions utilisées par les cibles sont déclarées dans ce type de bout de code: elles sont ajoutées à un fichier dans le dossier de travail `_targets_r` (différent du dossier `_targets` qui contient les cibles).
...
```{targets X, tar_simple=TRUE, echo=FALSE, tar_interactive = FALSE}
runifpoint(NbPoints)
```

Le bout de code précédent a pour entête `{targets X, tar_simple=TRUE}` pour stocker le jeu de points crée par le code dans la variable `X`.
...
La lecture du document est compliquée parce que la variable  de destination n'apparaît pas (elle est dans l'entête du bout de code).
Une solution est d'affecter explicitement la dernière valeur à une variable du même nom que la cible.
Pour la cible `NbPoints`, le code initial est: 
```{targets NbPoints_0, tar_simple=TRUE, eval=FALSE}
  1000
```

Il peut être remplacé avantageusement par:
```{targets NbPoints, tar_simple=TRUE, tar_interactive = FALSE}
  NbPoints <- 1000
```

La variable n'est utilisée que pour la lisibilité: elle n'a pas d'existence au delà du bout de code.
Elle pourrait avoir un autre nom (`z <- 1000`): la variable de destination de la cible a le nom du bout de code, c'est-à-dire `NbPoints`.
