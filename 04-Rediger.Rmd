...
La formule mathématique $l_s$ a été retirée du titre de la colonne parce que **flextable** ne l'interprète pas de façon simple (mais une solution existe[^413]).
...
[^413]: https://ardata-fr.github.io/flextable-gallery/gallery/

Pour un usage occasionnel, **flextable** est plus difficile à prendre en main que **kableExtra** mais il est plus puissant si un usage intensif des tableaux est nécessaire.
...
- `NbPoints`:
```{targets NbPoints, tar_simple=TRUE, eval=FALSE}
  1000
```
...
```{r, include=FALSE}
# Destruction des cibles déjà calculées pour la démonstration
tar_unscript()
```
